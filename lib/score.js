...
class ScoreSystem {
  ...
  rateAction (username, action, data, cb) {
    ...
    let tasks = co.wrap(function * () {
      ...
      if (bonus.newSkill) {
        skillsAdded = yield db.addSkill(username, bonus.skill)
      }

      if (bonus.badge) {
        badgesAdded = yield db.addBadge(username, bonus.skill)
      }
  ...
  onPushImage (achievements, data) {
    // la primera imagen subida tiene un boost de puntaje
    // tambien debe sumar una skill
    // empieza con 160
    let points = achievements.points

    let pointsToAdd = 10

    // Obtiene una skill de la lista
  ...
  skillGenerator (achievements, pointsToAdd) {
    ...
    // se asigna un contador al revez para enviar al usuario
    // informacion de cuantas imagenes faltan para el siguiente nivel
    ...
    // se asigna un contador al revez para enviar al usuario
    // informacion de cuantos puntos faltan para el siguietne nivel
